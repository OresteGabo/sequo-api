...
| [ ] | Partial | Courier reports problem | Mission can be moved to problem with reason. | `DeliveryMissionController` and `RelayParcelController` persist role-protected problem reports with actor, metadata, and idempotency. Delivery mission support can now requeue pre-pickup problem missions or cancel them with an auditable resolution record; notifications and broader investigation workflow remain. |
