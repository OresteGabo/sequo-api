...
| [ ] | Partial | Sequo collects from sellers | Sequo/courier missions collect each seller package into consolidation custody. | `POST /api/consolidations/{manifestId}/seller-packages/{subOrderId}/collected` records each authorized collection and automatically advances the manifest to `InSequoCustody` after the last package; direct mission-event synchronization remains. |
