...
| [ ] | Partial | Grouped Sequo consolidation | Multi-seller or programmed orders pass through Sequo and become one customer-facing package. | `SequoConsolidationService` and `ConsolidationPersistenceService` validate seller packages, persist readiness and Sequo custody, create the final package ID, and expose idempotent final customer-package dispatch; customer tracking and relay destination selection remain. |
