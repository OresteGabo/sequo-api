...
| [ ] | Partial | Logistics platform | Sequo API owns logistics orchestration across customer, seller, courier, relay, and admin workflows. | `DeliveryMissionService`, `DeliveryReadinessDispatchService`, `DeliveryMissionController`, `DeliveryTrackingController`, `DeliveryPinService`, and `OrderDeliveryLifecycleService` cover ready-suborder dispatch, mission assignment/list/detail, courier ownership, hashed PIN creation/validation, proof-backed transitions, delivered order state, return-window opening, relay release validation, pre-pickup reassignment, admin cancellation/problem states, and proof-redacted customer tracking; dispatch scheduling, ETA, broader workflow notifications, and settlement integration remain. |
