...
| [ ] | Partial | Logistics platform | Sequo API owns logistics orchestration across customer, seller, courier, relay, and admin workflows. | `DeliveryMissionService`, `DeliveryReadinessDispatchService`, `DeliveryMissionController`, `DeliveryTrackingController`, and `DeliveryPinService` cover ready-suborder dispatch, mission assignment/list/detail, courier ownership, hashed PIN creation/validation, proof-backed transitions, relay release validation, pre-pickup reassignment, admin cancellation/problem states, and proof-redacted customer tracking; dispatch scheduling, ETA, workflow notifications, and settlement integration remain. |
...
- [ ] Notification outbox worker plus after-commit hooks from order, merchant fulfillment, delivery, relay, return, refund, and settlement workflows. Generic after-commit enqueue and routed outbox worker are implemented; workflow-specific event publishers remain.
