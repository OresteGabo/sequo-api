...
| [ ] | Partial | Relay logistics | Parcels staying more than 2 weeks can start storage fees. | `RelayParcelPolicy` identifies eligibility and `RelayParcelApplicationService` persists delayed/review status after evaluation; fee calculation exists, while scheduler, fee ledger, tariff decision, and notification workflow remain. |
...
| [ ] | Partial | Event-driven triggers | Fire notifications from committed domain events/outbox, not controller side effects. | `NotificationOutboxService`, `NotificationOutboxEventListener`, and `NotificationOutboxWorker` provide idempotent after-commit enqueue, ready-event scanning, worker leases, routed inbox/delivery planning, retry backoff, and final failure state; workflow-specific publishers and provider delivery remain. |
...
- [ ] Settlement scheduler for merchant payout eligibility within 1 week and shortfall ledger posting. Merchant payout and ledger persistence, idempotent accrual/shortfall/adjustment writes, secured read APIs, admin eligibility evaluation, and monitoring counts are implemented; an automatic scheduled runner and provider payout execution remain.
