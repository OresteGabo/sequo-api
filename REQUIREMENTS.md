...
| [ ] | Partial | Logistics | Amazon-like/non-urgent items can take days and should be handled through Sequo-controlled consolidation. | `GroupedSequo` order route, persistent consolidation manifests, seller readiness confirmations, and `ProgrammedConsolidation` missions exist; automatic custody completion and final dispatch remain. |
...
| [ ] | Partial | Wallets | Integrate Yas Togo and Moov Africa digital wallets. | Provider abstractions exist; signed, replay-protected webhook intake is persisted for Yas Togo and Moov Africa, while real provider API adapters, payment-state reconciliation, and production credentials remain. |
...
| [ ] | Partial | Apps and roles | Support separate customer, seller, courier, and future relay apps against one API. | Merchant fulfillment, delivery mission, relay, customer tracking, support, and admin routes now enforce role groups and scoped reads; persisted merchant memberships and broader app-specific claims remain incomplete. |
...
| [ ] | Partial | Consolidation | Multi-merchant purchases pass through Sequo and arrive as one package to the customer. | Persistent manifests, seller readiness confirmation, Sequo custody transitions, and one delivery mission per seller package are implemented; final-package dispatch and customer tracking integration remain. |
| [ ] | Partial | Merchant fulfillment | Merchant prepares package in a reasonable delay and marks it ready for Sequo pickup. | `MerchantFulfillmentService` records seller-response and packing deadlines, detects overdue sub-orders, publishes acceptance/rejection/preparing/ready and idempotent SLA-warning outbox events for linked customer orders, persists idempotent support escalations for overdue sellers, and `MerchantFulfillmentController` exposes scoped list/detail/SLA plus accept/prepare/pack/handoff/reject/SLA-warning scan/admin escalation endpoints; paid order wiring creates idempotent merchant sub-orders, while persisted account-to-merchant mapping and automatic cancellation policy remain. |
...
| [ ] | Partial | Refunds | Refund is triggered only after Sequo physically receives the returned item. | `ReturnPersistenceService` persists refund-pending state only after Sequo physical receipt and handles idempotent refund triggers; wallet provider execution remains. |
| [x] | Implemented | Settlements | Merchant payout should happen within 1 week after Sequo receives package/custody. | `SettlementLedgerService`, `SettlementPersistenceService`, and `SettlementController` accrue and persist merchant payouts, expose scoped payout reads, promote eligible payouts, and inspect immutable ledger entries. Provider payout execution remains future integration work. |
| [ ] | Partial | Logistics platform | Sequo API owns logistics orchestration across customer, seller, courier, relay, and admin workflows. | `DeliveryMissionService`, `DeliveryReadinessDispatchService`, `DeliveryMissionController`, `DeliveryTrackingController`, `DeliveryPinService`, `CourierAvailabilityService`, `CustomerPickupConfirmationService`, and `OrderDeliveryLifecycleService` cover ready-suborder dispatch with persisted run history and opt-in scheduling, click-collect pickup confirmation, customer order list/detail reads scoped to the authenticated owner, mission assignment/list/detail, courier ownership, courier pause/unpause guards, hashed PIN creation/validation, persisted idempotency keys for mobile retries, proof-backed transitions, delivered order state, return-window opening, merchant settlement accrual, relay release validation, pre-pickup reassignment, stale mission expiry/no-show problem scans, audited support requeue/cancel resolutions, admin cancellation/problem states, and proof-redacted customer tracking; ETA, live courier offer queue, broader workflow notifications, and broader settlement execution remain. |
...
| [ ] | Partial | SMS fallback | Use SMS backup for critical PIN, blocked delivery, relay pickup, and urgent payment/refund events. | `NotificationChannelPolicy` gates SMS behind critical events, explicit fallback, persisted user preference, push failure/unavailability, and remaining budget. Paid provider adapter is intentionally not wired yet. |
...
| [ ] | Partial | Rider radar | Publish live mission offers and high-priority subscriber orders to eligible riders. | `RiderRadarRealtimeService` publishes mission-offer payloads to distinct eligible rider user queues; automatic eligible-rider discovery and subscriber-priority feed wiring remain. |
| [ ] | Partial | Bargaining realtime | Publish active offer/counter/accept/reject updates to bargaining participants. | `BargainingRealtimeService` publishes session updates to customer and merchant realtime queues; direct integration with persisted bargaining workflows remains. |
| [ ] | Partial | Event-driven triggers | Fire notifications from committed domain events/outbox, not controller side effects. | `NotificationOutboxService`, `NotificationOutboxEventListener`, `NotificationOutboxWorker`, and the opt-in outbox scheduler provide idempotent after-commit enqueue, ready-event scanning, worker leases, routed inbox/delivery planning, retry backoff, and final failure state. Merchant fulfillment, merchant SLA warning, delivery completion, assigned delivery problem, relay delay/review, return request/PIN, Sequo return receipt, and refund-trigger workflows publish committed events; broader workflow-specific publishers and provider delivery remain. |
...
- [ ] Settlement scheduler for merchant payout eligibility within 1 week and shortfall ledger posting. Merchant payout and ledger persistence, idempotent accrual/shortfall/adjustment writes, secured read APIs, admin/opt-in scheduled eligibility evaluation, and monitoring counts are implemented; provider payout execution remains.
