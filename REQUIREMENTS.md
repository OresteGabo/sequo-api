...
| [ ] | Partial | Logistics platform | Sequo API owns logistics orchestration across customer, seller, courier, relay, and admin workflows. | `DeliveryMissionService`, `DeliveryMissionController`, `DeliveryTrackingController`, and `DeliveryPinService` cover mission assignment/list/detail, courier ownership, hashed PIN creation/validation, proof-backed transitions, relay release validation, pre-pickup reassignment, admin cancellation/problem states, and proof-redacted customer tracking; dispatch scheduling, ETA, workflow notifications, and settlement integration remain. |
