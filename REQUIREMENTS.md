...
| [ ] | Partial | Merchant fulfillment | Merchant prepares package in a reasonable delay and marks it ready for Sequo pickup. | `MerchantFulfillmentService` records seller-response and packing deadlines, detects overdue sub-orders, publishes acceptance/rejection/preparing/ready outbox events for linked customer orders, and `MerchantFulfillmentController` exposes scoped list/detail/SLA plus accept/prepare/pack/handoff/reject; paid order wiring creates idempotent merchant sub-orders, while persisted account-to-merchant mapping and SLA warning automation remain. |
