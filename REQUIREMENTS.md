...
| [ ] | Partial | Logistics platform | Sequo API owns logistics orchestration across customer, seller, courier, relay, and admin workflows. | `DeliveryMissionService` and `DeliveryMissionController` persist and expose mission assignment, proof-backed transitions, relay release validation, and problem states; dispatch, tracking, and settlement integration remain. |
