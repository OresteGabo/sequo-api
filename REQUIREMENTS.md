...
| [ ] | Partial | Logistics platform | Sequo API owns logistics orchestration across customer, seller, courier, relay, and admin workflows. | `DeliveryMissionService`, `DeliveryMissionController`, and the secured `DeliveryPinService` cover mission assignment, hashed delivery PINs, proof-backed transitions, relay release validation, and problem states; PIN endpoint integration, dispatch, tracking, and settlement integration remain. |
