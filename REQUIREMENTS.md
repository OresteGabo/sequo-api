...
| [ ] | Partial | Relay logistics | Customers can choose relay pickup for eligible deliveries. | `PointDeRelai` route exists; relay availability, parcel assignment, and customer pickup flow remain. |
...
| [ ] | Not implemented | Relay logistics | Pickup uses numeric code or QR code plus identity validation. | Requires hashed pickup code, QR payload, attempt limits, and relay endpoint. |
...
| [ ] | Partial | Merchant fulfillment | Merchant prepares package in a reasonable delay and marks it ready for Sequo pickup. | Merchant ready endpoint is specified; SLA tracking and service implementation remain. |
...
| [ ] | Partial | Logistics platform | Sequo API owns logistics orchestration across customer, seller, courier, relay, and admin workflows. | Domain map/API docs cover the surface; many controllers and persistence modules remain. |
