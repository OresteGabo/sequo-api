...
| [ ] | Partial | Relay logistics | Parcels staying more than 2 weeks can start storage fees. | `RelayParcelPolicy` and `RelayParcelService` identify delayed parcels; scheduler, fee ledger, and notification workflow remain. |
...
| [ ] | Partial | In-app inbox | Persist notification history with read/archive state. | `NotificationReadService` lists an owner's messages and supports read/archive/unarchive with ownership filtering; HTTP APIs and pagination remain. |
...
- [ ] Social identity linking table to safely handle Google login and password login for the same verified email. `SocialIdentity` and `V4__create_social_identity_table.sql` add provider-subject uniqueness; account-link workflow integration remains.
